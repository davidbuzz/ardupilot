//https://raw.githubusercontent.com/ChristianVisintin/termiWin/master/termiWin.c
/* termiWin.c
*
...
int tcsetattr(int fd, int optional_actions, const struct termios* termios_p) {
  ...
  EchoOpt = EchoOpt; //unused
  l_opt = l_opt; //unused
  tostop = tostop; //unused
...
int readFromSerial(int fd, char* buffer, int count) {
  ...
  if (fd != com.fd) return -1;
  long unsigned int rc = 0; //LPDWORD is really a (long unsigned int*)
  int ret;
...
int openSerial(char* portname, int opt) {
  ...
  if (strlen(portname) > 4) {
    ...
    com.port = (char *)calloc(1, portSize); // calloc returns a void* that we must cast to what we use it for
#ifdef _MSC_VER
  ...
  //COMx
    ...
    com.port = (char *)calloc(1, portSize);
  }
