...
{
    if (!_dev) {
        DEV_PRINTF("ICM dbg: probe no device\n");
        return nullptr;
    }
    ...
    if (sensor == nullptr) {
        DEV_PRINTF("ICM dbg: probe alloc failed\n");
        return nullptr;
    }

    if (!sensor->hardware_init()) {
        DEV_PRINTF("ICM dbg: hw_init failed who42=0x%02x who456=0x%02x tries=%lu ok=%lu\n",
    ...
    DEV_PRINTF("ICM dbg: probe ok who42=0x%02x who456=0x%02x\n",
               (unsigned)rp_inv3_probe_last_whoami_42,
               (unsigned)rp_inv3_probe_last_whoami_456);
...
void AP_InertialSensor_Invensensev3::start()
{
    ...
    if (!_imu.get_gyro_instance(gyro_instance) || !_imu.get_accel_instance(accel_instance)) {
        DEV_PRINTF("ICM dbg: start failed to get gyro/accel instance\n");
        return;
    }
    ...
    DEV_PRINTF("ICM dbg: start ok type=%u backend_hz=%u sample_hz=%u\n",
               (unsigned)inv3_type,
               (unsigned)backend_rate_hz,
...
bool AP_InertialSensor_Invensensev3::check_whoami(void)
{
    ...
    DEV_PRINTF("ICM dbg: whoami miss who42=0x%02x who456=0x%02x\n",
               (unsigned)rp_inv3_probe_last_whoami_42,
               (unsigned)rp_inv3_probe_last_whoami_456);
    return false;
}
...
bool AP_InertialSensor_Invensensev3::hardware_init(void)
{
    ...
    if (!check_whoami()) {
        DEV_PRINTF("ICM dbg: hardware_init check_whoami failed\n");
        return false;
    }
