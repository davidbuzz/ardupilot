...
// when we call SDMMC_HOST_DEFAULT it sets host.slot == 1 , whic is the 4 bit slot used by MMC, and the only one available so we always use it.
#define SPI_DMA_SLOT    host.slot
#else
#define SPI_DMA_SLOT 1 // fallback to 1 anyway 
//#error "Cant use hardcoded DMA channel any more, please set it to 0=disabled,1,2, or 3=auto"
#endif
...
void mount_sdcard_spi()
{
    ...
    if (!initialized) {
        ...
        ret = spi_bus_initialize((spi_host_device_t)host.slot, &bus_cfg, SPI_DMA_SLOT);
        if (ret != ESP_OK) {
            initialized = false;
