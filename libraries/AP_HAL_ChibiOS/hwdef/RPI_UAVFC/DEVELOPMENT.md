...
**Every `no symbol match` line is a real miss, and fails the firmware build.**
The bootloader build is exempt, as most entries are absent there. The entries
that used to raise it harmlessly - the `__stats_*` ones, which need
...
generated `hwdef.h` and stays quiet when it is off. It still fails when the
define is on and the symbol is absent, so the marker suppresses the expected
case without blunting the check. Tag any new entry the same way.
