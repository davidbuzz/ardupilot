...
/*===========================================================================*/
    ...
    */
    ...
    // on pico 2 a gpio numb is just a u-int-8 that we do a palSetLineMode/PAL_MODE_RESET on.
    //
    // HAL_PWM_GPIO_LINES is generated by chibios_hwdef.py when PWM pins are defined.
    // It expands to a comma-separated list of PAL_LINE(IOPORT1,n) values.
    #if defined(HAL_PWM_GPIO_LINES)
    static const ioline_t pico2_pwm_gpio_lines[] = {HAL_PWM_GPIO_LINES};
    #define PICO2_NUM_PWM_LINES (sizeof(pico2_pwm_gpio_lines)/sizeof(pico2_pwm_gpio_lines[0]))
    #endif
...
/*===========================================================================*/
    ...
    static void pico2_gpio_init(void) {
        ...
        #if defined(HAL_PWM_GPIO_LINES) && defined(PICO2_NUM_PWM_LINES)
        for (unsigned i = 0; i < PICO2_NUM_PWM_LINES; i++) {
            palSetLineMode(pico2_pwm_gpio_lines[i], PAL_MODE_ALTERNATE_PWM);
        }
        #endif
    }
#endif
