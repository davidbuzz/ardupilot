...
void pico2_gpio_init(void) {
    ...
    /* Set the tone-alarm buzzer pin to PWM alternate function (FUNCSEL=4).
     * ALARM pins are declared with GPIO(n)+ALARM tags in hwdef.dat rather than
     * PWM(n), so chibios_hwdef.py does NOT include them in HAL_PWM_GPIO_LINES.
     * Without this explicit palSetLineMode() the PWM slice hardware is active
     * after pwmStart() but the signal cannot reach the GPIO pad — the buzzer
     * stays silent even though AP_ToneAlarm is driving the PWM CC register.
     * HAL_PWM_ALARM_GPIO_LINE is emitted by chibios_hwdef.py alongside the
     * RP_PWM_USE_PWMn define for the alarm slice. */
    #if defined(HAL_PWM_ALARM_GPIO_LINE)
    palSetLineMode(HAL_PWM_ALARM_GPIO_LINE, PAL_MODE_ALTERNATE_PWM);
    #endif

    /* Treat hwdef GPIO(x) pins like OUTPUT for RP2350 bring-up.
     * HAL_GPIO_PINS entries are generated from hwdef GPIO(n) tags and can be
     * controlled through AP_HAL::GPIO. On RP2350, pads reset to FUNCSEL=31
     * (NULL/high-Z), so writes to these lines are ineffective unless we switch
     * to SIO output mode first.
     *
     * Keep PWM-backed GPIO entries out of this pass because they must remain in
     * alternate PWM mode unless runtime channel mapping explicitly repurposes
     * them as GPIO. */
    ...
    for (unsigned i = 0; i < sizeof(gpio_lines) / sizeof(gpio_lines[0]); i++) {
      ...
      /* Do not clobber an ALARM-capable buzzer pin back to SIO output.
       * Laurel uses GPIO(80) on PA5 as a PWM-backed alarm source, and the
       * tone backend relies on RCOutput later restoring PWM FUNCSEL there. */
  ...
  /* Configure board buzzer control pin as SIO push-pull output.
   * Keep the default output latch LOW to match hwdef OUTPUT LOW and avoid
   * emitting a startup chirp before AP_Notify/AP_Buzzer takes ownership. */
  ...
  /* Note: motor outputs are intentionally left in PAL_MODE_ALTERNATE_PWM,
   * configured at the top of this function. For RP2350, alternate-function
   * pin mode enables the peripheral output path; forcing PAL_MODE_OUTPUT_
   * PUSHPULL here would clobber PWM FUNCSEL and break motor output routing. */
}
...
void __early_init(void) {
      ...
      stm32_gpio_init();
    #endif
    #if defined(STM32_HW) && !defined(HAL_XIP_ENABLED) || defined(HAL_FORCE_CLOCK_INIT)
...
void __late_init(void) {
   ...
   * The assignment is deferred to the post-halInit()#if defined(RP2350)
