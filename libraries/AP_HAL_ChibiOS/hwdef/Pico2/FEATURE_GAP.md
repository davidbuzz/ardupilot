...
| Board ID | AP_Bootloader_USBCDC | ✅ | ✅ | ✅ | ⚠️ partial — USB CDC TX not yet confirmed | `hwdef-bl.dat` implemented: `FLASH_RESERVE_START_KB 0`, `FLASH_BOOTLOADER_LOAD_KB 32`, `APP_START_OFFSET_KB 32`. BL at `0x10000000`, storage `0x10008000`–`0x1000FFFF`, app at `0x10010000`. USB (`SIOD0`/OTG1) + UART0 (`PA12/PA13`). `HAL_USE_EFL TRUE`, `HAL_USE_WDG TRUE`, `HAL_USE_SIO TRUE`. Board enumerates correctly: `lsusb` shows 'ArduPilot BootLoader'. USB CDC TX (BL → host) not yet verified working. SOF-based flush present but suspected unreliable on RP2350; `bl_usb_tx_poll_drain()` added to `cout()` as direct obqueue poll-flush. WIP diagnostic: `usb_test_thread` emits A-Z bytes; `cat /dev/ttyACM0` should show cycling letters if CDC TX works. |
| Board ID | AP_Bootloader | ✅ | ✅ | ✅ | ⚠️ partial — uploader.py protocol not yet confirmed | `hwdef-bl.dat` implemented. Guards added: `bl_protocol.cpp` (`rccDisable*`, `rccResetOTG_FS` behind `#if !defined(RP2350)`), `support.cpp` (`DBGMCU_BASE` stub for RP2350), `chibios_hwdef.py` (`SIOD%u` for RP2350 `BOOTLOADER_DEV_LIST`). Keys: flash_base `0x10000000` (XIP), 64KB block erase, 15s BL timeout. `uploader.py --port /dev/ttyACM0 arducopter.apj`: **not yet confirmed working** — USB CDC TX (INSYNC+OK responses from BL) has not been verified flowing to host. Prerequisite: confirm `cat /dev/ttyACM0` shows A-Z from WIP test thread; then verify `cin()` receives uploader sync bytes. |
