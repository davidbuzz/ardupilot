...
** current hardware issue - main app, if left long enough, doesnt appear to reach a point where _initialized = TRUE , meaning that the setup() has never completed and so we've never entered main(), but we're still not getting data out of our cdc-usb serial port, ie no mavlink or console. maybe its configured wrong, maybe its code bug preventing output? is this the same issue as we had in the bootloader,.?
