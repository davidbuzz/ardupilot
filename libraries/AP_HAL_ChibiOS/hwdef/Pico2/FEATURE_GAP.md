...
|----------|---------|-----------|-------|--------|--------------------|-------|
| Serial / UART | USB serial (SERIAL0) | OTG1 | OTG1 | ✅ | ✅ VERIFIED hardware | Working. `HAL_USE_USB TRUE`, `HAL_USE_SERIAL_USB TRUE`, `USBv1` ChibiOS driver. USB CDC confirmed working: /dev/ttyACM1 appears, MAVLink heartbeats flow, 777 parameters fetchable, STATUSTEXT messages received. NOTE: host must assert DTR=True (`ser.dtr = True` in pyserial, or use mavproxy which does it automatically) for data to flow on Linux. |
...
| Board ID | AP_Bootloader_USBCDC | ✅ | ✅ | ✅ | ✅ hardware testing DONE | `hwdef-bl.dat` implemented: `FLASH_RESERVE_START_KB 0`, `FLASH_BOOTLOADER_LOAD_KB 32`, `APP_START_OFFSET_KB 32`. BL at `0x10000000` (32KB), storage `0x10008000`–`0x1000FFFF` (32KB), app at `0x10010080`. USB (`SDU1`/OTG1) + UART0 (`PA12/PA13`). `HAL_USE_EFL TRUE`, `HAL_USE_WDG TRUE`, `HAL_USE_SIO TRUE`. `lsusb` shows `ArduPilot Pico2-BL`. **Fix applied:** `setup_usb_strings()` call added to `rp2350_imagedef_ref.c` `__late_init()` so USB string descriptors are populated; USB now enumerates in ~3s (was 17s). BL port is `/dev/ttyACM1` (ttyACM0 = CMSIS-DAP debugprobe). |
