...
# NOTE: Standalone Laurel RP2350 board makefile.
##############################################################################
...
# ArduPilot provides ff_memalloc/ff_memfree in hwdef/common/malloc.c and a
# GPS-based get_fattime() in hwdef/common/stm32_util.c. Remove the ChibiOS
# copies to avoid duplicate-symbol link failures when FATFS is enabled.
...
# ArduPilot provides better (DMA-capable, size-limited) ff_memalloc/
# ff_memfree in hwdef/common/malloc.c, and a GPS-based get_fattime() in
# stm32_util.c. Remove the duplicate ChibiOS objects to keep FATFS linkable.
ALLCSRC := $(filter-out $(CHIBIOS)/os/various/fatfs_bindings/fatfs_syscall.c, $(ALLCSRC))
ALLCSRC := $(filter-out $(CHIBIOS)/os/various/fatfs_bindings/fatfs_diskio.c, $(ALLCSRC))
...
# hal_safety.c is omitted in the non-smart hal.mk branch; add it explicitly.
...
UDEFS = $(ENV_UDEFS) $(FATFS_FLAGS) -DHAL_BOARD_NAME=\"$(HAL_BOARD_NAME)\" \
				-DHAL_MAX_STACK_FRAME_SIZE=$(HAL_MAX_STACK_FRAME_SIZE) -DRP2350=1 \
