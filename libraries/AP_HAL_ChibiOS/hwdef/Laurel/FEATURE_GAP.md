...
| Filesystem / Logging | MAVFTP baseline on Laurel | Mature on STM32 targets | Should match standard ArduPilot behavior | ⚠️ | ⚠️ partial | Retested 2026-04-07 over Laurel USB CDC after two firmware-side fixes in `GCS_FTP.cpp` (RP2350 reply tx-buffer gate reduced from 33 to 8 bytes, and path termination switched to request-size boundary). MAVLink transport remains healthy (stable HEARTBEAT stream, FTP capability bit set), but MAVFTP is still not fully reliable: `list @SYS` still returns intermittent `ListDirectory failed, generic error` from pymavlink CLI. |
| Filesystem / Logging | MAVFTP `get @SYS/threads.txt` | Works on mainstream targets | Must work on Laurel | ⚠️ | ⚠️ partial | Retested 2026-04-07 after the above `GCS_FTP.cpp` fixes: still failing on Laurel. `python3 -m pymavlink.mavftp ... get @SYS/threads.txt` continues to return `get failed, generic error`, and no output file is created. |
| Filesystem / Logging | MAVFTP `get @SYS/tasks.txt` second-attempt behavior | Works on mainstream targets | Must not require retry hacks | ⚠️ | ⚠️ partial | Retested 2026-04-07 after the above `GCS_FTP.cpp` fixes: first and second `get @SYS/tasks.txt` still fail on Laurel with `get failed, generic error`, and both local output files remain absent. |
