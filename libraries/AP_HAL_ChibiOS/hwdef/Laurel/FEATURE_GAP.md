...
|----------|---------|-----------|--------|--------|--------------------|-------|
| Serial / UART | USB serial (`SERIAL0`) | OTG1 | OTG1 | ✅ | ✅ done | Verified on Laurel hardware: stable MAVLink over `/dev/ttyACM0` at 115200 with repeated HEARTBEAT/SYS_STATUS and STATUSTEXT traffic during 2026-04-05 bring-up captures. |
...
| SPI | SPI0 IMU bus | SPI1/SPI4 etc. | GPIO2/GPIO4/GPIO3 with CS on GPIO1 | ✅ | ⚠️ partial | Runtime has two modes: without active SWD traffic this bus can initialize, but during SWD debug sessions IMU probing can fail because CS shares GPIO1 with SWDIO (`HAL_GPIO_PIN_ICM42688_CS` and `HAL_GPIO_PIN_SWDIO` are both `PAL_LINE(IOPORT1,1U)` in generated `hwdef.h`). |
| SPI | ICM42688P onboard IMU | Board-specific SPI IMU | SPI0 + GPIO1 CS | ✅ | ⚠️ partial | Recent GDB captures on 2026-04-05 show `rp_inv3_probe_attempt_count=1`, `rp_inv3_probe_last_whoami_42=0x00`, `rp_inv3_probe_success_count=0`, and backend fallback to `AP_InertialSensor_NONE` when SWD is active. Re-test with SWDIO physically disconnected from GPIO1 to confirm normal IMU WHOAMI/health path. |
...
| CPU / Architecture | Dual-core RP2350 model | Single-core STM32 | Core0 ChibiOS + core1 dispatcher | ⚠️ | ❌ not done | Laurel keeps `CH_CFG_SMP_MODE FALSE`. `RP_CORE1_START` is temporarily set to `FALSE` for bring-up because SWD captures repeatedly showed deadlock in RP2350 `start_core1()` FIFO handshake on cold start. Core1 launch needs a robust fix before re-enabling. |
