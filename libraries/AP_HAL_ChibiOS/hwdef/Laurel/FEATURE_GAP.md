...
| Filesystem / Logging | MAVFTP baseline on Laurel | Mature on STM32 targets | Should match standard ArduPilot behavior | ✅ | ✅ done | Fixed 2026-04-09: root cause was ChibiOS scheduling starvation — the FTP worker thread ran at `PRIORITY_IO` (58), but on RP2350 `CH_CFG_TIME_QUANTUM=0` (pure priority-preemptive, no round-robin) combined with UART threads waking every 1 ms at prio 60 meant the worker never got CPU. Fix: raise worker to `PRIORITY_UART+1` (61) on RP2350, well above the UART/LED/NET cluster (60) but below I2C (176). Verified: `ListDirectory @SYS` returns full `Fthreads.txt / Ftasks.txt / ...` listing over USB CDC in ≤2 s. |
| Filesystem / Logging | MAVFTP `get @SYS/threads.txt` | Works on mainstream targets | Must work on Laurel | ✅ | ✅ done | Fixed 2026-04-09 by the priority-fix commit. `OpenFileRO @SYS/threads.txt` returns ACK with file size 100000 bytes over USB CDC. Full ReadFile sequence verified 2026-04-09: `memory.txt` returns 66 bytes (MemInfoV1, START/LEN/FREE/LRG/TYPE fields) via complete OpenFileRO→ReadFile→TerminateSession flow. `tasks.txt` returns 8-byte header ("TasksV2\n") on first fetch (by design: enables RECORD_TASK_INFO), full task data appears ≥10 s later after scheduler's 0.1-Hz `update_logging` task allocates `_task_info`. |
| Filesystem / Logging | MAVFTP `get @SYS/tasks.txt` second-attempt behavior | Works on mainstream targets | Must not require retry hacks | ✅ | ✅ done | Fixed 2026-04-09 by the priority-fix commit. All `@SYS/*.txt` files (tasks, dma, memory, uarts, timers, threads) now return ACK on first attempt with no retry needed. |
