...
| SPI | SPI0 IMU bus | SPI1/SPI4 etc. | GPIO2/GPIO4/GPIO3 with CS on GPIO1 | ✅ | ✅ done | Verified on Laurel hardware: repeated `EKF3 IMU0 initialised` and `EKF3 IMU0 forced reset` status messages confirm live IMU backend operation on the configured SPI0 bus. |
| SPI | ICM42688P onboard IMU | Board-specific SPI IMU | SPI0 + GPIO1 CS | ✅ | ✅ done | Verified on Laurel hardware during 2026-04-05 runtime capture. The onboard IMU is detected and used by EKF3 (no inertialsensor-none fallback path observed). |
