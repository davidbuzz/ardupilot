...
| Notify / Board IO | Blue status LED | Board LED | GPIO6 | ⚠️ | ❌ not done | Runtime diagnostics in OpenOCD show GPIO6 CTRL is not staying in expected SIO mode (`FUNCSEL=5`): observed `0x00010000` (FUNCSEL=0 with override bits) and `0x00000000` in failing boots. Manual force-write to SIO mode via SWD can drive the pin, but firmware ownership/polarity remains unresolved. |
| Notify / Board IO | Green status LED | Board LED | GPIO7 | ⚠️ | ❌ not done | Runtime diagnostics in OpenOCD show GPIO7 CTRL being muxed to SPI (`0x00000001`, FUNCSEL=1 `SPI0_TX`) instead of expected SIO GPIO mode. Indicates a post-init pin-mux override; LED behavior cannot be considered verified until this override is fixed. |
