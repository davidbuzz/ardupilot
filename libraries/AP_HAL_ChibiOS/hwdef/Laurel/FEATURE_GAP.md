...
| Filesystem / Logging | MAVFTP baseline on Laurel | Mature on STM32 targets | Should match standard ArduPilot behavior | ⚠️ | ⚠️ partial | Retested 2026-04-07 over Laurel USB CDC: MAVLink transport is healthy (repeated HEARTBEATs, AUTOPILOT_VERSION capabilities `0x000000000000f9ef`, FTP capability bit set), but MAVFTP is still unreliable. `list @SYS` intermittently succeeds in MAVProxy startup path yet often returns `ListDirectory failed, generic error` from pymavlink CLI. `get` paths remain failing (see rows below). |
| Filesystem / Logging | MAVFTP `get @SYS/threads.txt` | Works on mainstream targets | Must work on Laurel | ⚠️ | ⚠️ partial | Retested 2026-04-07: still failing on Laurel. `python3 -m pymavlink.mavftp ... get @SYS/threads.txt` reproducibly returns `get failed, generic error`. MAVProxy `ftp get @SYS/threads.txt` reports command dispatch but does not create the destination file (`/tmp/laurel_threads_mp.txt` absent). |
| Filesystem / Logging | MAVFTP `get @SYS/tasks.txt` second-attempt behavior | Works on mainstream targets | Must not require retry hacks | ⚠️ | ⚠️ partial | Retested 2026-04-07: first and second `get @SYS/tasks.txt` both fail on Laurel. Pymavlink CLI returns `get failed, generic error` on each attempt. MAVProxy command path also failed to materialize output files (`/tmp/laurel_tasks_mp1.txt`, `/tmp/laurel_tasks_mp2.txt` absent). |
...
| CPU / Architecture | Remove non-standard `serial_hello` thread | Not present in STM32 path | Laurel should use existing UART/USB threads (`OTG1`, `UART_RX`) | ⚠️ | ⚠️ partial | Code path updated 2026-04-07: ad-hoc `SERHELLO` background thread removed from `AP_HAL_ChibiOS` startup. USB CDC/MAVLink retest on 2026-04-07 shows no transport regression (stable HEARTBEAT flow and normal autopilot telemetry), but `@SYS/threads.txt` cannot yet be fetched due ongoing MAVFTP get failures, so direct runtime proof that `SERHELLO` disappeared from thread listings is still pending. |
