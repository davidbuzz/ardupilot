#!/usr/bin/env python
'''
these tables are generated from the NXP datasheet for the   RT1062
...
mcu = {
    ...
    #  OCRAM = On-Chip RAM Memory Controller ( part of flexram module) 
    #      There is 1 OCRAM controller implemented in the chip. One controller is for up to 512KB on-chip RAM.
    #      Designed as a simple on-chip memory controller, it supports only one AXI port with memory banks.
    #      The OCRAM block is implemented as an slave module on the 64-bit system AXI bus
    #  FlexRAM - This SoC has 512 KB of on-chip RAM which is shared by I-TCM, D-TCM and general
    #            purpose On Chip RAM (OCRAM). The FlexRAM is the manager of the big on-chip
    #            RAM array.
    #          - 64-bit I-TCM interface and 2x 32-bit D-TCM interface.
    #          -   RAM Array portioning. RAM size of ITCM, DTCM and OCRAM can be configured from 0 to full 
    #                  RAM Array size separately
    #  OCRAM, DTCM and ITCM share 512 KB of on-chip RAM. SW can configure from 0 to
    #         512 KB full RAM array size for OCRAM, DTCM and ITCM by IOMUX GPR16 and
    #         GPR17. The RAM size step is 32KB
