            ...
            if dev.startswith('OTG2'):
                ...
                if self.is_rp_mcu():
                    ...
                    if not self.intdefines.get('HAL_UART_NODMA', 0):
                        # dma_rx/dma_tx set to false: RP2350 SIO UART DMA is not yet
                        # validated on hardware; enabling it causes Shared_DMA::call_wait()
                        # deadlock in UARTDriver::_begin() during AP_SerialManager::init().
                        # Stream/channel IDs are retained for future DMA enablement.
                        f.write(
...
''')
        ...
        if alarm is not None:
            ...
            if self.is_rp_mcu():
                # RP2350 sys_clk is typically 375 MHz (or 150 MHz default).  The PWM
                # CH_DIV register has only an 8-bit integer field (max 255), so the
                # minimum achievable PWM clock = sys_clk / 256.  At 375 MHz that floor
                # is ~1.46 MHz, so a 1 MHz pwm_clock cannot be represented (divisor
                # 375 > 255).  Use 1.5 MHz: divisor = 375/1.5 = 250, fits in 8 bits at
                # 375 MHz and also works at 150 MHz (divisor = 100).
                pwm_clock = 1500000
                period = 1000  # initial period; actual tone periods are set dynamically
                # RP2350 PWMConfig only has 2 channels (PWM_CHANNELS=2) and no
                # trailing platform-specific fields (unlike STM32 which has extra
                # timer pointer fields after channels[]).  Emit a minimal 2-channel
                # initializer with no trailing zeros.
        ...
        if self.is_rp_mcu():
            ...
            if alarm_timer_num is not None:
                ...
                # ALARM pins are not in HAL_PWM_GPIO_LINES (they use GPIO(n)+ALARM tags,
                # not PWM(n)), so pico2_gpio_init() would otherwise skip this pin and
                # leave it at SIO/NULL FUNCSEL — the PWM slice runs but the signal
                # cannot reach the pad and the buzzer produces no sound.
