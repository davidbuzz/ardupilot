...
void Util::toneAlarm_set_buzzer_tone(float frequency, float volume, uint32_t duration_ms)
{
    ...
    if (_toneAlarm_pwm_group.pwm_drv != nullptr) {
        if (is_zero(frequency) || is_zero(volume)) {
            ...
            /* On RP2350, AP_Notify::Buzzer::init() calls hal.gpio->pinMode(HAL_BUZZER_PIN)
             * which sets FUNCSEL=5 (SIO), overwriting the FUNCSEL=4 (PWM) set by
             * pico2_gpio_init().  Re-assert PWM alternate function here every time a
             * tone starts so the PWM slice signal actually reaches the pad. */
            palSetLineMode(HAL_PWM_ALARM_GPIO_LINE, PAL_MODE_ALTERNATE_PWM);
#endif
#ifdef HAL_BUZZER_NOMINAL_FREQ_HZ
            /* Some electromagnetic transducers (e.g. RDTE-4.000-3030-NS1) require a
             * specific drive frequency for rated SPL.  Override the musical note
             * frequency to the transducer's nominal value while preserving the
             * on/off timing pattern so notifications remain audibly distinct. */
