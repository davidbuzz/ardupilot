 ...
 *  2. The C implementation is __RAMFUNC2__
(lives in SRAM, copied there at
...
/*
 * Core fault capture implementation — __RAMFUNC2__
so it runs from SRAM and
 ...
 *
 * __RAMFUNC2__
is critical: COPY_VECTORS_TO_RAM copies the vector POINTER TABLE
 * to SRAM and redirects VTOR there, so the vector index fetch is flash-free.
 * But without __RAMFUNC2__
the handler CODE still lives in XIP flash — the CPU
 * would fetch bad instructions if XIP is unreliable at the higher clock, causing
 * a double-fault lockup before any C runs.  __RAMFUNC2__
ensures both the vector
