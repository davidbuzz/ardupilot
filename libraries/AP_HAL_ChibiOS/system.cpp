 ...
 *
 * Problem with the original handler: it called memcpy() from XIP flash to
 * capture the exception frame.  If the fault was caused by unreliable XIP
 * access (e.g. overclocked sys_clk with a too-aggressive flash divider) then
 * the memcpy itself faults, producing a double-fault which locks the CPU at
 * PC=0xEFFFFFFE before any C code can run.
 *
 * Solution:
 *  1. A __attribute__((naked)) entry trampoline uses inline asm to determine
 *     whether the faulting code was using PSP or MSP and passes the correct
 *     stack-frame pointer to the C implementation — no C stack is touched.
 *  2. The C implementation is __RAMFUNC__ (lives in SRAM, copied there at
 *     startup), so it executes even if XIP flash is completely unreliable.
 *  3. All registers are written into a static global struct `fault_info` in
 *     SRAM.  After a halt, GDB command `print fault_info` shows everything.
 *     OpenOCD can also read it: `mdw <address of fault_info> 16`.
 *  4. BKPT #0 is issued so GDB auto-halts; no need to set a breakpoint first.
 *
 * To inspect after a fault in GDB:
 *   print fault_info
 *   print/x fault_info.cfsr
 *   print/x fault_info.pc
 */
...
__RAMFUNC__ void fault_capture(uint32_t *frame, uint32_t exc_return);

/*
 * Core fault capture implementation — __RAMFUNC__ so it runs from SRAM and
 * is immune to XIP flash reliability issues at overclocked frequencies.
 *
 * @param frame      Pointer to the hardware exception frame (from PSP or MSP)
 * @param exc_return EXC_RETURN value (LR on handler entry), used to determine
...
__RAMFUNC__ void fault_capture(uint32_t *frame, uint32_t exc_return)
{
    ...
    /* BKPT only when a debugger is connected — without one, BKPT escalates to
     * HardFault which (since we are already in HardFault) causes a double-fault
     * lockup at 0xEFFFFFFE, defeating the whole handler.  With a debugger,
     * OpenOCD auto-halts and `print fault_info` shows everything. */
...
/*
 * Naked trampolines: determine PSP vs MSP from EXC_RETURN bit[2], then
 * tail-call fault_capture().  The naked attribute means the compiler emits
 * no prologue/epilogue, so no stack space is used before we save the frame.
 *
 * __RAMFUNC__ is critical: COPY_VECTORS_TO_RAM copies the vector POINTER TABLE
 * to SRAM and redirects VTOR there, so the vector index fetch is flash-free.
 * But without __RAMFUNC__ the handler CODE still lives in XIP flash — the CPU
 * would fetch bad instructions if XIP is unreliable at the higher clock, causing
 * a double-fault lockup before any C runs.  __RAMFUNC__ ensures both the vector
 * entry AND the code it points to are in SRAM.
 *
 * The "b fault_capture" branch relies on both the trampoline and fault_capture
 * being in the same .ramtext section (VMA in RAM), keeping the offset within
 * the 16 MB range of the Thumb-2 unconditional branch instruction.
 */
__RAMFUNC__ void HardFault_Handler(void) __attribute__((naked));
__RAMFUNC__ void HardFault_Handler(void) {
    __asm volatile (
 ...
 * Without SCB->SHCSR fault-enable bits set (ChibiOS doesn't set them by
 * default) these escalate to HardFault anyway, but having explicit handlers
 * gives cleaner CFSR decoding. */
__RAMFUNC__ void BusFault_Handler(void) __attribute__((naked));
__RAMFUNC__ void BusFault_Handler(void) {
    __asm volatile (
        "tst    lr, #4          \n"
        "ite    eq              \n"
        "mrseq  r0, msp         \n"
...
__RAMFUNC__ void UsageFault_Handler(void) __attribute__((naked));
__RAMFUNC__ void UsageFault_Handler(void) {
    __asm volatile (
        "tst    lr, #4          \n"
        "ite    eq              \n"
        "mrseq  r0, msp         \n"
...
__RAMFUNC__ void MemManage_Handler(void) __attribute__((naked));
__RAMFUNC__ void MemManage_Handler(void) {
    __asm volatile (
        "tst    lr, #4          \n"
        "ite    eq              \n"
        "mrseq  r0, msp         \n"
...
#else  /* !PIC02_AVAILABLE — legacy STM32 fault handlers */
