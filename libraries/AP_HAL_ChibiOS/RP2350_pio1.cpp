...
PIO1Owner pio1_intended_owner(void)
{
    ...
    // without the PIO OSD built, OSD_TYPE 6 has no backend to give the block to
    ...
    /*
      ...
      an MSP one.
     */
    if ((got && is_equal(osd_type, float(PIO1_OSD_TYPE))) ||
        (got2 && is_equal(osd_type2, float(PIO1_OSD_TYPE)))) {
