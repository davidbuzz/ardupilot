...
// and bdshot_prepare_for_next_pulse has no equivalent there
    ...
    for (uint8_t i = 0; i < 4; i++) {
        ...
        const uint16_t chan_packet = (group.chan[i] == chan || chan == RCOutput::ALL_CHANNELS) ? packet : zero_packet;
#if defined(RP2350)
        // the PIO takes the packet as-is; there is no DMA buffer to fill
        RCOutput_pico::write_frame(group.chan[i], chan_packet);
#else
        fill_DMA_buffer_dshot(group.dma_buffer + i, 4, chan_packet, group.bit_width_mul);
#endif
    }
