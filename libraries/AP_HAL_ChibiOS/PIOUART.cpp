...
void PIORXDriver::_configure_gpio(uint8_t pin, bool is_output)
{
    ...
    // For inverted-UART protocols (SBUS, option OPTION_RXINV): set GPIO INOVER=01
    // so the PIO SM sees standard UART levels (idle HIGH) from the idle-LOW SBUS
    // wire signal.  MUST be written AFTER palSetPadMode() because that function
    // writes the full IO_BANK0 GPIOx_CTRL register and would clear INOVER.
    // IO_BANK0 GPIOx_CTRL address = 0x40028004 + pin*8.
    // INOVER at bits [17:16]: 0b00=normal, 0b01=invert, 0b10=force-low, 0b11=force-high.
...
void PIORXDriver::_start_rx_sm(uint32_t int_div, uint32_t frac_div)
{
    ...
    // Force the RX SM PC to the correct program entry point.  After restart,
    // SM PC defaults to 0 which runs the TX program (pull block) and stalls.
    // Execute an unconditional JMP to the selected RX program offset.
    ...
    while (!(pio->FSTAT & (1u << (PIO_FSTAT_RXEMPTY_LSB + sm)))) {
        ...
        if (_readbuf && _initialized) {
            if (!sbus_sanitize) {
                ...
                // SBUS on PIOUART: assemble full frames and only forward valid
                // 25-byte packets. This keeps framing garbage out of the upper
                // protocol layer and improves failsafe stability.
                ...
                if (_sbus_rx.ofs == 25U) {
                    ...
                    if (footer_ok && flags_ok) {
                        // Debounce single-frame SBUS failsafe-flag spikes caused by
                        // occasional UART framing noise: require 3 consecutive flagged
                        // frames before forwarding FAILSAFE bit to upper layers.
                        if (flags & (1U << 3)) {
...
void PIORXDriver::_begin(uint32_t b, uint16_t rxSpace, uint16_t txSpace)
{
    ...
    // Also re-initialize if OPTION_RXINV changed (e.g. RC detection cycling
    // through SBUS vs non-inverted configs at the same baud rate).
