...
bool UARTDriver::set_options(uint16_t options)
{
    ...
    // Updating _flow_control here (before begin() is called) ensures that when
    // _begin() subsequently calls set_flow_control(_flow_control), it picks up
    // the requested state.  Calling set_flow_control() directly handles the case
    // where set_options() is invoked on an already-running port.
    if (arts_line != 0) {
