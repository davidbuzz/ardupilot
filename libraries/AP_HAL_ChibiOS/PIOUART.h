...
// Instruction memory layout (17 of 32 words used per PIO):
//   [0..4]   TX program (8N1 transmit, side-set)
//   [5..9]   Standard RX program (8N1/8N2, no parity skip)
//   [10..18] SBUS RX program (8E2, parity-bit skip + stop-bit validation)
...
//   0x8020: push  noblock
//
// Standard 8N1/8N2 receiver: no parity bit, returns to wait immediately after
// the push.  Allows back-to-back bytes with a single stop bit.
static const uint16_t k_pio_uart_rx_pgm[PIO_UART_RX_PROG_LEN] = {
    0x2020u, 0xEA27u, 0x4001u, 0x0647u, 0x8020u,
};

// SBUS RX: 8 cycles/bit, pre-relocated for offset 10. Adds parity-bit skip
// and validates first stop bit before pushing to FIFO.
//
// Absolute instruction addresses:
//   10: wait  0 pin, 0              -- start bit (LOW after INOVER on SBUS wire)
//   11: set   x, 7             [10] -- delay to bit-0 centre, set counter
//   12: in    pins, 1               -- sample one bit
//   13: jmp   x--, 12         [6]   -- loop 8 times (target=12)
//   14: mov   y, y            [6]   -- nop 7 cycles: stall through parity bit
//   15: jmp   pin, 17              -- stop bit must be MARK (HIGH)
//   16: jmp   10                   -- framing error: drop byte and resync
//   17: push  noblock              -- deliver byte
//   18: jmp   10                   -- back to wait (target=10)
//
// Timing from start-bit edge: push occurs at cycle 76; the 7-cycle nop plus
// 1-cycle jmp consume cycles 77-83, placing the SM back at wait at cycle 85.
// The SBUS parity bit spans cycles 72-79 and the first stop bit 80-87, so
// the SM is safely busy through the parity bit and re-enters wait well within
// the first stop bit.  Stop bits are MARK (HIGH after INOVER) so wait 0 pin 0
// holds until the next start bit edge at cycle 96 (back-to-back bytes). ✓
//
// static_assert guards hardcoded absolute jump targets.
