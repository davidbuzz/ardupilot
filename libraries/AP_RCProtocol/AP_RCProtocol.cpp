...
bool AP_RCProtocol::new_input()
{
    ...
    if (!pulse_protocol_active) {
        // Once a pulse-based protocol is actively providing input there is no
        // value in polling the other backends here; detect_async_protocol()
        // will only consider alternates again after should_search() says input
        // has gone idle. Skipping the extra backend work avoids starving USB
        // CDC on Pico2 while GPIO pulse bursts are arriving.
        for (uint8_t i = 0; i < ARRAY_SIZE(backend); i++) {
            if (backend[i] != nullptr) {
                backend[i]->update();
            }
